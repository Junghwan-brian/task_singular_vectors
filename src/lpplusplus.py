...
def compute_centroids_alpha(z_s: torch.tensor,
    ...
    centroids = (one_hot*z_s / one_hot.sum(-2, keepdim=True)
                 ).sum(1)  # [batch, K, d]
    return centroids
...
def compute_centroids(z_s: torch.tensor,
    ...
    """
    one_hot = get_one_hot(
        y_s, num_classes=y_s.unique().size(0)).transpose(1, 2)
...
def init_lp(features, labels, text_weights, shot):
    centroids = compute_centroids(features.unsqueeze(
        0), labels.unsqueeze(0))  # [batch, num_class, d]

    num_classes = text_weights.shape[1]
    classifier = nn.Linear(
        features.shape[1], num_classes, bias=True).to(features)
    ...
    # init_alpha
    final_init_alpha_mean = calculate_init_alpha(
        features, labels, shot, text_weights)

    alpha_vec = torch.autograd.Variable(
        final_init_alpha_mean * torch.ones(1, num_classes).to(features), requires_grad=True)
