...
def maybe_dictionarize(batch):
    ...
    if len(batch) == 2:
        ...
        if len(batch[0]) == 2:  # Two transforms
            batch = {'images': batch[0][0], 'images_': batch[0]
                     [1], 'labels': batch[1], 'index': batch[2]}
        else:
...
def get_dataloader(dataset, is_train, args, image_encoder=None):
    if image_encoder is not None:
        feature_dataset = FeatureDataset(
            is_train, image_encoder, dataset, args.device)
    ...
    if isinstance(args.subsample, float) and is_train:
        ...
        print(
            f"Subsampling dataloader from size {len(src)} to size {lengths[0]}.")
