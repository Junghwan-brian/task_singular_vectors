...
def mask_multiply(coefs, mask, params):
    if params.ndim != 3:
        return (coefs*0.).sum()
    if params.ndim == 1:
        # Classic block wise for 1-dim parameters
...
class WeightedImageEncoder(nn.Module):
    def __init__(self, model, task_vectors, blockwise=True, partition=None) -> None:
        ...
        self.dparams = [[tv.vector[k] for k in tv.vector]
                        for tv in task_vectors]
        self.blockwise = blockwise
        self.partition = partition
        if self.partition is not None:
            self.mask_mats = {}
            self.coef = torch.nn.Parameter(torch.zeros(
                len(task_vectors), len(self.params), self.partition))
            for p in self.params:
                mask = torch.randint(self.partition, p.shape)
                self.mask_mats[p.shape] = torch.nn.Parameter(
                    torch.nn.functional.one_hot(mask).moveaxis(-1, 0).half(), requires_grad=False)
        elif blockwise:
    ...
    def _apply(self, fn):
        ...
        if hasattr(self, 'mask_mats'):
            new_self.mask_mats = {k: fn(v)
                                  for k, v in new_self.mask_mats.items()}
        return new_self
    ...
    def forward(self, x) -> torch.Tensor:
        if self.partition is not None:
            dparams = [mask_multiply(self.coef[:, i,], self.mask_mats[dp[0].shape], torch.cat(
                [d.unsqueeze(0) for d in dp], dim=0)) for i, dp in enumerate(zip(*self.dparams))]
        elif self.blockwise:
            dparams = [sum([p * c[i] for p, c in zip(dp, self.coef)])
                       for i, dp in enumerate(zip(*self.dparams))]
        else:
            dparams = [sum([p * c for p, c in zip(dp, self.coef)])
...
class WeightedLinearizedModel(nn.Module):
    def __init__(self, model, task_vectors, blockwise=True) -> None:
        ...
        self.dparams = [[tv.vector[k] for k in tv.vector if k.startswith(
            'model.params.')] for tv in task_vectors]
        self.blockwise = blockwise
        if blockwise:
            self.coef = torch.nn.Parameter(torch.zeros(
                len(task_vectors), len(self.params0)))
        else:
            self.coef = torch.nn.Parameter(torch.zeros(len(task_vectors),))
