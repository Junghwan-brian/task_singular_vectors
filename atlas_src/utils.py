...
import torch
import tqdm
from atlas_src.datasets.common import maybe_dictionarize
...
def get_n_shots(dataset, shots, n_class, args):
    ...
    data_loader = torch.utils.data.DataLoader(
        index_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8)
...
def get_preds(dataset, model, args):
    ...
    all_preds = - \
        torch.ones((len(dataset), model.module.classification_head.out_features))
    trusted = torch.zeros(len(dataset), dtype=torch.bool)
    with torch.no_grad():
        for i, batch in enumerate(tqdm.tqdm(data_loader)):
            batch = maybe_dictionarize(batch)
            preds = model(batch["images"].cuda())
            all_preds[batch["index"]] = torch.nn.functional.softmax(
                preds, dim=-1).to(all_preds)
    return all_preds
...
class TIPWrapper(torch.nn.Module):
    ...
    def forward(self, x, tv_logits=None, feats=None):
        ...
        cache_logits = (
            (-1) * (self.beta_alpha[0] - self.beta_alpha[0] * affinity)).exp() @ self.labels.to(affinity)
...
class LPPWrapper(torch.nn.Module):
    ...
    def forward(self, x, tv_logits=None, feats=None):
        ...
        logits = vision_logits + \
            torch.ones(feats.shape[0], 1).to(
                feats) @ self.alpha_vec.to(feats) * tv_logits / 100
        return logits
